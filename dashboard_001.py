...
class DashboardCommunicator:
    ...
    def request_ticker_data(self, ticker: str):
        ...
        try:
            ...
            current_symbols = st.session_state.user_session.preferences.get('symbols', [])
            if ticker not in current_symbols:
                current_symbols.append(ticker)
                
                # Update preferences
                updated_preferences = st.session_state.user_session.preferences.copy()
                updated_preferences['symbols'] = current_symbols
                updated_preferences['real_time_enabled'] = True
                
                self.auth_manager.save_user_preferences(user_id, updated_preferences)
            ...
            self.supabase_client.table('data_requests').insert(request_data).execute()
            ...
            st.success(f"Requested data for {ticker}. Publisher will fetch options data shortly.")
    ...
    def get_options_data(self, ticker: str) -> List[Dict]:
        ...
        try:
            response = self.supabase_client.table('options_data')\
                .select('*')\
    ...
    def get_model_results(self, ticker: str) -> List[Dict]:
        ...
        try:
            response = self.supabase_client.table('model_results')\
                .select('*')\
        ...
        try:
            if time_to_expiry <= 0:
                return {
                    ...
                    'breakeven_price': strike + option_purchase_price if option_type == 'C' else strike - option_purchase_price,
            ...
            final_prices = current_price * np.exp((risk_free_rate - 0.5 * volatility**2) * dt + volatility * np.sqrt(dt) * Z)
...
def show_api_keys_setup():
    ...
    with st.form("api_keys_form"):
        ...
        binance_api_key = st.text_input("Binance API Key", value=current_keys.get('binance_api_key', ''), type="password")
        binance_secret_key = st.text_input("Binance Secret Key", value=current_keys.get('binance_secret_key', ''), type="password")
        
        st.subheader("Alpaca API Keys")
        alpaca_api_key = st.text_input("Alpaca API Key", value=current_keys.get('alpaca_api_key', ''), type="password")
        alpaca_secret_key = st.text_input("Alpaca Secret Key", value=current_keys.get('alpaca_secret_key', ''), type="password")
        
        st.subheader("Financial Modeling Prep API Key")
        fmp_api_key = st.text_input("FMP API Key", value=current_keys.get('fmp_api_key', ''), type="password")
...
def show_options_analysis():
    ...
    has_binance_keys = api_keys.get('binance_api_key') and api_keys.get('binance_secret_key')
    
    if not has_binance_keys:
        st.warning("⚠️ Please configure your Binance API keys in the Settings to access full functionality.")
    ...
    with col2:
        if st.button("🔍 Request Data", disabled=not ticker_input):
            if ticker_input:
                st.session_state.dashboard_comm.request_ticker_data(ticker_input.upper())
    
    # Show options data if available
    if ticker_input:
        options_data = st.session_state.dashboard_comm.get_options_data(ticker_input)
        ...
        if options_data:
            ...
            if not df.empty:
                ...
                with col1:
                    risk_free_rate = st.number_input("Risk-free Rate", value=0.05, min_value=0.0, max_value=1.0, format="%.4f")
                with col2:
                    default_volatility = st.number_input("Default Volatility", value=0.25, min_value=0.0, max_value=5.0, format="%.4f")
                with col3:
                    transaction_cost = st.number_input("Transaction Cost", value=10.0, min_value=0.0)
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        if 'expiry' in row and row['expiry']:
                            ...
                            time_to_expiry = (expiry_date - datetime.now(timezone.utc)).days
                        else:
                            time_to_expiry = 30  # Default to 30 days
                        
                        # Use implied volatility if available, otherwise default
                        volatility = row.get('implied_volatility', default_volatility) or default_volatility
                        ...
                        enhanced_row = {
                            ...
                            'Expected P&L': f"${pl_stats['expected_pl']:.2f}",
                            'Max Loss': f"${pl_stats['max_loss']:.2f}",
                        ...
                        st.warning(f"Error processing option {row.get('symbol', 'unknown')}: {e}")
                        continue
                ...
                if enhanced_data:
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        option_type_filter = st.selectbox("Filter by Type:", ["All", "C", "P"])
                    with col2:
                        min_probability = st.slider("Min P&L Probability:", 0.0, 1.0, 0.0, 0.05)
                    with col3:
                        min_days = st.slider("Min Days to Expiry:", 0, 365, 0)
                    ...
                    prob_values = [float(p.strip('%'))/100 for p in filtered_df['P&L Probability']]
                    filtered_df = filtered_df[np.array(prob_values) >= min_probability]
                    filtered_df = filtered_df[filtered_df['Days to Expiry'] >= min_days]
                    ...
                    if len(filtered_df) > 0:
                        ...
                        with col1:
                            ...
                            fig_prob = px.bar(
                                filtered_df,
                                x='Strike',
                                y=[float(p.strip('%'))/100 for p in filtered_df['P&L Probability']],
                                color='Type',
                        ...
                        with col2:
                            ...
                            expected_pl_values = [float(pl.strip('$')) for pl in filtered_df['Expected P&L']]
                            fig_pl = px.histogram(
                                x=expected_pl_values,
                                nbins=20,
...
def show_black_scholes_heatmap():
    ...
    with st.sidebar:
        ...
        days_to_maturity = st.number_input('Time to Maturity (days)', value=365)
        Risk_Free_Rate = st.number_input('Risk-Free Interest Rate', value=0.1)
        ...
        transaction_cost = st.number_input("Opening/Closing Cost", value=0.0)
    ...
    with tab2:
        ...
        st.metric("Expected P&L (Current Parameters)", f"${specific_contract_pl:.2f}")
    ...
    with tab3:
        st.write('Calculate the expected distribution of the underlying asset price, option premium and P&L from trading')
        ...
        if trade_type == 'Call':
            option_prices = np.maximum(simulation_paths[dynamic_index, :] - SelectedStrike, 0)
        else:
            option_prices = np.maximum(SelectedStrike - simulation_paths[dynamic_index, :], 0)
        ...
        with col1:
            ...
            fig_price.add_vline(x=SelectedStrike, line_color="red", annotation_text="Strike Price")
        ...
        with col2:
            ...
            fig_pl.add_vline(x=0, line_color="red", annotation_text="Breakeven")
            st.plotly_chart(fig_pl, use_container_width=True)
...
def real_time_data_fragment():
    ...
    if not st.session_state.get('user_session') or not st.session_state.user_session.preferences.get('real_time_enabled', False):
        return
    ...
    for symbol in symbols[:3]:  # Limit to first 3 symbols
        try:
            ...
            model_results = st.session_state.dashboard_comm.get_model_results(symbol)
            ...
            if model_results:
                ...
                with col1:
                    st.metric(f"{symbol} - Model", latest_result.get('model_name', 'N/A'))
                with col2:
                    st.metric("Result", f"${float(latest_result.get('result', 0)):.2f}")
                with col3:
                    timestamp = pd.to_datetime(latest_result.get('timestamp', datetime.now()))
...
def main():
    ...
    if "dashboard_comm" not in st.session_state:
        st.session_state.dashboard_comm = DashboardCommunicator(st.session_state.auth_manager)
    
    # Check authentication
    if not st.session_state.user_session or not st.session_state.user_session.is_authenticated:
    ...
    if st.session_state.user_session.preferences.get('real_time_enabled', False):
    ...
    if page == "🏠 Home":
        ...
        with col1:
            ...
            st.metric("Real-time Updates", "On" if st.session_state.user_session.preferences.get('real_time_enabled') else "Off")
        
        with col2:
            st.subheader("🔧 Your Settings")
            risk_free_rate = st.session_state.user_session.preferences.get('risk_free_rate', 0.05)
            st.metric("Risk-Free Rate", f"{risk_free_rate:.2%}")
            volatility = st.session_state.user_session.preferences.get('default_volatility', 0.25)
        ...
        if symbols:
            ...
            for symbol in symbols:
                with st.expander(f"📈 {symbol}"):
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button(f"Request {symbol} Data", key=f"req_{symbol}"):
                            st.session_state.dashboard_comm.request_ticker_data(symbol)
                    with col2:
                        options_count = len(st.session_state.dashboard_comm.get_options_data(symbol))
